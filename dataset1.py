...
class FWIDataset(Dataset):
    ...
    Args:
        ...
        preload: whether to load the whole dataset into memory (please put this to false if working with small memory compute)
...
if __name__ == '__main__':
    transform_data = Compose([
        T.LogTransform(k=1),
        T.MinMaxNormalize(T.log_transform(-27.11, k=1), T.log_transform(55.10, k=1))
    ])
    transform_label = Compose([
        T.MinMaxNormalize(1500, 4500)
    ])
    dataset = FWIDataset(f'split_files/train.txt', transform_data=transform_data, transform_label=transform_label, file_size=500)
